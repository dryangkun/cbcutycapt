...
// Modification (C) 2012 Peter Wilson-Ferrer / Integraclick LLC 
// <peter.wilson-ferrer@integraclick.com>
//
// Modification adds ability to specify a max-width and max-height 
// for screenshots.
//
...
//
// Integraclick, LLC, hereby disclaims all copyright
// interest in the program `CutyCapt'
// (which takes screenshots of websites) written 
// by Bjoern Hoehrmann.
//
// signature of Chris Maynard, 3 December 2012
// Chris Maynard, Director of Development
// $Id$
//
////////////////////////////////////////////////////////////////////
...
// method to set the max width/height dimensions passed in the CLI 
void
CutyCapt::setMaxDimensions(int width, int height) {
    mMaxWidth = width;
    mMaxHeight = height;
}
...
CutyCapt::saveSnapshot() {
  ...
  switch (mFormat) {
    ...
    default: {
      ...
      // If the max width and max height args were passed (and were also greater
      // than 0, then use those for saving the screenshot. If they were not set
      // or passed as 0, use the actual pixel dimensions of the page.
      if (mMaxWidth != 0 && mMaxHeight != 0) {
          width = mMaxWidth;
          height = mMaxHeight;
      } else {
          QSize size = mPage->viewportSize();
          std::stringstream qWidth;
          std::stringstream qHeight;
          qWidth << size.width();
          qWidth >> width;
          qHeight << size.height();
          qHeight >> height;
...
//  "  --out-quality=<int>            Output format quality from 1 to 100          \n"
    ...
    "  --max-width=<int>              Max width for the image (default: 0 (unltd)) \n"
    "  --max-height=<int>             Max height for the image (default: 0 (unltd))\n"
...
//  "  --user-styles=<url>            Location of user style sheet (deprecated)    \n"
    ...
    " Max-width and Max-height added by                                            \n"
    "                 Peter Wilson-Ferrer / Integraclick LLC 2012                  \n"
    "                 - peter.wilson-ferrer@integraclick.com                       \n"
    "");
}
...
main(int argc, char *argv[]) {
  ...
  for (int ax = 1; ax < argc; ++ax) {
    ...
    } else if (strncmp("--max-width", s, nlen) == 0) {
      argMaxWidth = (unsigned int)atoi(value);
      
    } else if (strncmp("--max-height", s, nlen) == 0) {
      argMaxHeight = (unsigned int)atoi(value);
  ...
  if (argMaxWidth > 0 && argMaxHeight > 0) {
      main.setMaxDimensions(argMaxWidth, argMaxHeight);
  }
